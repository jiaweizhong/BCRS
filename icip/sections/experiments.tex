...
\midrule
(1) ESOD~\cite{ESOD}       & 0.344 & 0.171 & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
...
(4) Dual-Concat            & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
(5) Dual-Gated             & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
(6) Dual+SABL              & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
(7) HESOD (+ISPP)          & \textbf{0.371} & \textbf{0.195} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
\bottomrule
...
\textbf{Method / Config} & \shortstack{\textbf{mAP}\\\textbf{@.5}} & \shortstack{\textbf{mAP}\\\textbf{@.5:.95}} & \textbf{BPR} & \textbf{GFLOPs} & \textbf{FPS} \\
\midrule
(1) ESOD~\cite{ESOD}       & 0.750 & 0.320 & 0.947 & \textbf{202.4} & \textbf{85.7} \\
(2) Semantic-only$^*$      & 0.769 & 0.325 & \underline{0.991} & 266.8 & 73.5 \\
(3) Spectral-only$^*$      & 0.770 & \textbf{0.327} & \textbf{0.992} & 267.4 & 71.7 \\
(4) Dual-Concat            & \textbf{0.772} & \underline{0.326} & 0.986 & 281.2 & 69.8 \\
(5) Dual-Gated             & 0.765 & 0.324 & 0.990 & 261.5 & 74.3 \\
(6) Dual+SABL              & \underline{0.771} & 0.323 & 0.990 & 263.7 & 73.4 \\
(7) HESOD (+ISPP)          & 0.769 & 0.323 & 0.990 & \underline{209.1} & \underline{77.1} \\
\bottomrule
...
\midrule
(1) ESOD~\cite{ESOD}       & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
(2) Semantic-only$^*$      & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
(3) Spectral-only$^*$      & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
(4) Dual-Concat            & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
(5) Dual-Gated             & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
(6) Dual+SABL              & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
(7) HESOD (+ISPP)          & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
\bottomrule
...
\midrule
(1) ESOD~\cite{ESOD}       & 74.03\% & 86.78\% & 94.74\% & 79.35\% & 84.42\% \\
(2) Semantic-only$^*$      & 75.49\% & \underline{91.57\%} & \underline{95.71\%} & 81.94\% & 87.75\% \\
(3) Spectral-only$^*$      & 75.23\% & \textbf{91.69\%} & \textbf{95.89\%} & \textbf{86.45\%} & 87.77\% \\
(4) Dual-Concat            & 76.00\% & 91.50\% & 95.68\% & 80.00\% & \underline{87.84\%} \\
(5) Dual-Gated             & 75.49\% & 90.79\% & 95.50\% & \textbf{86.45\%} & 87.26\% \\
(6) Dual+SABL              & \underline{76.67\%} & 91.49\% & 94.77\% & 80.65\% & \textbf{87.89\%} \\
(7) HESOD (+ISPP)          & \textbf{77.19\%} & 90.70\% & 94.89\% & \underline{83.23\%} & 87.59\% \\
\bottomrule
...
\textbf{Effectiveness of Dual Evidence and Coverage Optimization.} Tables~\ref{tab:uavdt_ablation} and~\ref{tab:seaperson_ablation} dissect the component-wise contributions across benchmarks. On SeaPerson ($2048\times 2048$), replacing standard BCE with the object-level soft-coverage loss $\mathcal{L}_{\mathrm{cover}}$ (Arm 1 $\to$ 2) delivers an immediate \textbf{+1.9 pp} boost in $\mathrm{mAP@.5}$ and raises Bounding Patch Recall (BPR) from 0.947 to \textbf{0.991}, demonstrating that joint instance coverage optimization effectively prevents premature pruning of micro targets. Furthermore, the spectral-only selector (Arm 3) matches semantic-only accuracy (0.770 vs.\ 0.769 $\mathrm{mAP@.5}$, 0.992 BPR), proving that channel-pooled high-frequency structural gradients offer an orthogonal, dependable routing cue. Unconditional feature concatenation (Arm 4) achieves peak $\mathrm{mAP@.5}$ (\textbf{0.772}), outperforming learned gating (Arm 5) by avoiding over-suppression of candidate regions.

\textbf{Efficiency and Localization with ISPP and SABL.} Integrating scale-aware box regression in Arm (6) maintains high detection accuracy (0.771 $\mathrm{mAP@.5}$) while tightening patch clustering (263.7 GFLOPs, 73.4 FPS). Replacing the standard head with the inverted residual ISPP decoupled head (Arm 7) further reduces arithmetic complexity by \textbf{20.7\%} (263.7 $\to$ 209.1 GFLOPs) and cuts parameters to 25.9M, accelerating throughput to 77.1 FPS. Crucially, as shown in Table~\ref{tab:seaperson_size_buckets}, \emph{Very Tiny} recall peaks at \textbf{77.19\%} (+2,605 recovered micro-targets over baseline).
