# Vendored, unmodified, from the official TinyPerson benchmark toolkit:
# https://github.com/ucas-vg/PointTinyBenchmark/blob/TinyBenchmark/tiny_benchmark/maskrcnn_benchmark/data/datasets/evaluation/coco/cocoeval.py
# A fork of pycocotools' cocoeval.py extended (search "add by G" / "changed by hui")
# with TinyPerson-specific area bins (Params.EVAL_STRANDARD='tiny': tiny/tiny1-3/small/
# reasonable) and ignore/uncertain-region handling. Vendored directly (self-contained --
# only needs numpy + pycocotools) rather than installing the full maskrcnn_benchmark
# framework this file normally ships inside of, which pins PyTorch 1.0.1 and would
# conflict with this project's PyTorch 2.8 environment. See eval_tinyperson_official.py
# for the driver that uses this to reproduce the paper's own APt50/APs50 protocol.
__author__ = "tsungyi"
...
class COCOeval:
    ...
    def _prepare(self):
        ...
        for gt in gts:
            if self.use_ignore_attr:
                ...
                gt["ignore"] = ("iscrowd" in gt and gt["iscrowd"]) or gt[
                    "ignore"
                ]  # changed by hui
            else:
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        if len(ignore_gts_idx) > 0 and len(dt) > 0:
            ignore_gts_area = np.array(
                [g["area"] for g in gt if g["_ignore"]]
            )  # use area
                ...
                for dind, d in enumerate(dt):
                    ...
                    if m == -1:
                        ...
                        if self.use_iod_for_ignore and len(ignore_gts) > 0:
                            ...
                            iods = self.IOD_by_IOU(
                                np.array([d["bbox"]]),
                                None,
                                ignore_gts_area,
                                ignore_ious[dind : dind + 1, :],
                            )[0]
    ...
    def summarize(self):
            ...
            if ap == 1:
                ...
                if iouThr is not None:
                    t = np.where(float_equal(iouThr, p.iouThrs))[
                        0
                    ]  # t = np.where(iouThr == p.iouThrs)[0] changed by hui
                    s = s[t]
                s = s[:, :, aind, mind]
            if len(s[s > -1]) == 0:
                mean_s = -1
        ...
        def _summarizeDets():
            ...
            stats.extend(
                [
                    _summarize(1),
                    _summarize(1, iouThr=0.5, maxDets=self.params.maxDets[2]),
                    _summarize(1, iouThr=0.6, maxDets=self.params.maxDets[2]),
                    _summarize(1, iouThr=0.7, maxDets=self.params.maxDets[2]),
                    _summarize(1, iouThr=0.75, maxDets=self.params.maxDets[2]),
                    _summarize(1, iouThr=0.8, maxDets=self.params.maxDets[2]),
                    _summarize(1, iouThr=0.9, maxDets=self.params.maxDets[2]),
                ]
            )
            for i in range(n):
                ...
                stats.append(
                    _summarize(
                        1,
                        areaRng=self.params.areaRngLbl[i + 1],
    ...
    def setDetParams(self):
        ...
        if eval_standard.startswith("tiny"):
            ...
            if eval_standard == "tiny":
                ...
                raise ValueError(
                    "eval_standard is not right: {}, must be 'tiny' or 'tiny_sanya17'".format(
                        eval_standard
                    )
                )
