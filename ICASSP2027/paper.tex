...
% numbers on \name map each author to their \address line; $^{\ast}$ marks
% the corresponding author (Hui Zhang).
\name{Jiawei Zhong$^{1}$\thanks{Source code at \url{https://github.com/deepsota-ai/HESOD}.}, Sicheng Chen$^{2}$, DanDan Liu$^{3}$, Kang An$^{2}$, Hui Zhang$^{2}$\sthanks{Corresponding author.}}
\address{
