...
% marks co-first authors (Jiawei Zhong, Sicheng Chen); $^{\ast}$ (via
% \sthanks, which auto-generates a matching symbol) marks the
% corresponding author (Hui Zhang). Neither symbol is an official
% ICASSP/Paper-kit field -- both are informal, widely-used academic
% conventions implemented manually since spconf.sty's \thanks{} prints no
% mark of its own.
\name{Jiawei Zhong$^{1,\dagger}$\thanks{Source code at \url{https://github.com/deepsota-ai/HESOD}.}\thanks{These authors contributed equally to this work.}, Sicheng Chen$^{2,\dagger}$, DanDan Liu$^{3}$, Kang An$^{2}$, Hui Zhang$^{2}$\sthanks{Corresponding author.}}
\address{
