...
def test_tinyperson_official_evaluator_rejects_nonstandard_gt(tmp_path: Path):
    evaluator_path = (
        ROOT / "scripts/esod_baseline/tinyperson_eval/eval_tinyperson_official.py"
    )
    spec = importlib.util.spec_from_file_location(
        "eval_tinyperson_official", evaluator_path
    )
    assert spec and spec.loader
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    ...
    wrong_name.write_text(
        json.dumps({"images": [{}] * 816, "annotations": []}), encoding="utf-8"
    )
...
def test_two_arm_runner_is_exactly_r0_and_concat_isphead():
    runner = (ROOT / "scripts/esod_baseline/run_tinyperson_two_arm.sh").read_text(
        encoding="utf-8"
    )
