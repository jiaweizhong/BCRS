...
class SplitImage(object):
    ...
    def __get_sub_image_corners_by_pieces(
        self, image_size, pieces=(1, 1), overlap=(0, 0)
    ):
        """
        ...
        size, pieces, overlap = (
            np.array(image_size),
            np.array(list(pieces)),
            np.array(list(overlap)),
        )
    ...
    def __get_sub_image_corners_by_size(
        self, image_size, sub_image_size, overlap=(0, 0)
    ):
        """
        :param image_size: tuple, (w, h) of origin image .
        ...
        size, sub_image_size, overlap = (
            np.array(image_size),
            np.array(list(sub_image_size)),
            np.array(list(overlap)),
        )
    ...
    def get_sub_image_corners(
        self, image_size, pieces=None, sub_image_size=None, overlap=(0, 0)
    ):
        """
        ...
        if sub_image_size is not None:
            return self.__get_sub_image_corners_by_size(
                image_size, sub_image_size, overlap
            )
        elif pieces is not None:
            return self.__get_sub_image_corners_by_pieces(image_size, pieces, overlap)

    def get_sub_image_boxes(
        self,
        corner: np.ndarray,
        annos: np.ndarray,
        annos_area: np.ndarray,
        keep_overlap=0.7,
    ):
        """
        ...
        new_anno_area = np.sqrt(
            (annos[:, 2] - annos[:, 0]) * (annos[:, 3] - annos[:, 1])
        )
    ...
    def cut_image(
        ...
        if annos is not None:
            ...
            for corner in corners:
                sub_anno, keep = self.get_sub_image_boxes(
                    corner, annos, annos_area, anno_keep_overlap
                )
    ...
    def get_sub_image_corner_and_boxes(
        self,
        image_size,
        bboxes: np.ndarray,
        pieces=None,
        ...
        corners = self.get_sub_image_corners(
            image_size, pieces, sub_image_size, overlap
        )
        corners = corners.reshape((-1, 4))
        if len(bboxes) > 0:
            annos_area = np.sqrt(
            ...
            for corner in corners:
                sub_anno, keep = self.get_sub_image_boxes(
                    corner, bboxes, annos_area, anno_keep_overlap
                )
                sub_annos.append(sub_anno)
                sub_keeps.append(keep)
        else:
...
class MergeResult(object):
    ...
    def merge_result(self, corners, results, scores=None):
        ...
        if (
            no_empty_result > 1 and self.use_nms
        ):  # only when no_empty sub result > 1, need nms to merge in overlap area
            merge_result, keep = self.nms(merge_result, scores)
        return merge_result

    def merge_maskrcnn_benchmark_result(
        self, corners, results, im_scales=None, image_size=None
    ):
        import torch
...
class COCOSplitImage(SplitImage):
    ...
    def __turn_anno_to_sub_image_anno(
        self, bboxes, origin_annos, sub_anno_id, sub_image_id
    ):
        """
    ...
    def __turn_image_info_to_sub_image_infos(
        self, corner, image_info, sub_image_id, generate_sub_image: bool
    ):
        ...
        if generate_sub_image:
            ...
            new_image_info["file_name"] = "{}_{}_{}{}".format(
                f, corner[0], corner[1], ext
            )
        else:
    ...
    def get_sub_image_corners_and_annos(
        self, image_info, annos, sub_anno_id, sub_image_id, generate_sub_image
    ):
        if len(annos) == 0:
        ...
        corners, sub_annos, sub_keeps = super(
            COCOSplitImage, self
        ).get_sub_image_corner_and_boxes(
            image_size,
            boxes,
            self.pieces,
            self.sub_image_size,
            self.overlap,
            self.anno_keep_overlap,
        )

        sub_image_infos = []
        for i, (corner, sub_anno, sub_keep) in enumerate(
            zip(corners, sub_annos, sub_keeps)
        ):
            sub_image_info = self.__turn_image_info_to_sub_image_infos(
                corner, image_info, sub_image_id, generate_sub_image
            )
            sub_image_infos.append(sub_image_info)
            if len(sub_anno) > 0:
                orgin_anno = [annos[keep_id] for keep_id in sub_keep]
                sub_annos[i] = self.__turn_anno_to_sub_image_anno(
                    sub_anno, orgin_anno, sub_anno_id, sub_image_id
                )
            else:
                sub_annos[i] = []
            sub_image_id += 1
    ...
    def __load_coco_data_map(self, json_data):
        ...
        image_id_to_image_info = {
            image_info["id"]: image_info for image_info in image_infos
        }
        anno_id_to_anno = {anno["id"]: anno for anno in annotations}
        for anno in json_data["annotations"]:
            image_id_to_annos_id[anno["image_id"]].append(
                anno["id"]
            )  # an image_id in anno must contained in 'images'
            ...
            if os.path.isdir(dst_annotation_path):
                ...
                if self.pieces is not None:
                    ...
                    save_pth = os.path.join(
                        dst_annotation_path, "{}_pw{}_ph{}{}".format(f, pw, ph, ext)
                    )
                else:
                    sw, sh = self.sub_image_size
                    save_pth = os.path.join(
                        dst_annotation_path, "{}_sw{}_sh{}{}".format(f, sw, sh, ext)
                    )
            else:
    ...
    def cut_image_for_coco_json_dataset(
        self,
        src_annotation_path,
        dst_annotation_path=None,
        src_image_dir=None,
        dst_image_dir=None,
    ):
        ...
        image_ids, image_id_to_image_info, anno_id_to_anno, image_id_to_annos_id = (
            self.__load_coco_data_map(json_data)
        )
        ...
        for i, image_id in enumerate(image_ids):
            ...
            corners, sub_annos, sub_image_infos, sub_anno_id, sub_image_id = (
                self.get_sub_image_corners_and_annos(
            ...
            if dst_image_dir is not None:
                ...
                for corner, sub_image_info in zip(corners, sub_image_infos):
                    ...
                    sub_image.save(
                        os.path.join(dst_image_dir, sub_image_info["file_name"])
                    )
...
class COCOMergeResult(MergeResult):
    ...
    def __load_coco_data_map(self, corner_gts):
        ...
        image_id_to_image_info = {
            image_info["id"]: image_info for image_info in corner_gts["images"]
        }

        filename_to_merge_image_id = {
            image_info["file_name"]: image_info["id"]
            for image_info in corner_gts["old_images"]
        }
    ...
    def __save_file(self, json_data, src_path, dst_path):
        if os.path.isdir(dst_path):
            ...
            save_pth = os.path.join(
                dst_path,
                "{}_merge_nms{}{}".format(
                    f, self.nms_th if self.use_nms else "None", ext
                ),
            )
        else:
            save_pth = dst_path
    ...
    def __call__(self, corner_gt_file_path, src_det_file_path, dst_det_file_path=None):
        ...
        # create data map
        merge_image_ids, image_id_to_image_info, merge_image_id_to_image_ids = (
            self.__load_coco_data_map(corner_gts)
        )
        ...
        for merge_image_id, image_ids in merge_image_id_to_image_ids.items():
            for image_id in image_ids:
                merge_image_id_to_det_results[merge_image_id].append(
                    image_id_to_det_boxes[image_id]
                )
                merge_image_id_to_corners[merge_image_id].append(
                    image_id_to_image_info[image_id]["corner"]
                )
        ...
        for merge_image_id in tqdm(merge_image_id_to_det_results):
            ...
            for det_results in det_results_list:
                ...
                det_bboxes.append(
                    np.array(
                        [
                            xywh2xyxy(np.array(det_result["bbox"]))
            ...
            for bbox, old_det_result in zip(merge_boxes, old_det_results):
                det_result = self.__turn_det_result(
                    bbox, merge_image_id, old_det_result
                )
        ...
        if dst_det_file_path is not None:
            save_pth = self.__save_file(
                all_merge_det_results, src_det_file_path, dst_det_file_path
            )
