...
def merge_det_result(
    json_result_file, corner_gt_file, merged_gt_file, merge_nms_th=1.0
):
    ...
    _, merged_json_result_file = COCOMergeResult(use_nms=use_nms, nms_th=merge_nms_th)(
        corner_gt_file, json_result_file, os.path.split(json_result_file)[0]  # dir
    )
...
def evaluate_ap(
    ...
    coco_eval = COCOeval(
        coco_gt, coco_dt, iou_type, ignore_uncertain, use_iod_for_ignore
    )
    coco_eval.evaluate()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="evaluate AP and MR for Tiny Benchmark."
    )
    parser.add_argument(
        "--res", dest="res", required=True, help="result file of detetor."
    )
    parser.add_argument("--gt", dest="gt", required=True, help="ground-truth file.")
    parser.add_argument(
        "--merge-gt", dest="merge_gt", help="merged ground truth file.", default=""
    )
    parser.add_argument(
        "--metric", dest="metric", help="merged ground truth file.", default="all"
    )
    parser.add_argument(
        "--score-file", dest="score_file", help="output score file.", default=""
    )
    parser.add_argument(
        "--tmp-log", dest="tmp_log", help="temporal log file.", default=""
    )
    parser.add_argument(
        "--mr-ious",
        dest="mr_ious",
        help="iou th settings while evaluating MR.",
        default="0.25,0.5,0.75",
    )
    parser.add_argument(
        "--mr-sizes",
        dest="mr_sizes",
        help="size settings while evaluating MR.",
        default="tiny1,tiny2,tiny3,tiny,small,All",
    )
    parser.add_argument(
        "--detail",
        dest="detail",
        help="output detail info in result file",
        action="store_true",
    )
    ...
    if len(args.merge_gt) > 0:
        _, det_file = merge_det_result(
            args.res, args.gt, args.merge_gt, merge_nms_th=0.5
        )
        print(det_file)
    ...
    if metric == "mr" or metric == "all":
        ...
        setup_labels = [
            x.strip() for x in args.mr_sizes.split(",") if len(x.strip()) > 0
        ]
        evaluate_mr(
            det_file,
            gt_file,
