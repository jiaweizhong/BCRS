...
def re_evaluate_all(model_dir, excute_cmd):
    ...
    for iter_id in iter_ids:
        f = os.path.join(
            ".", model_dir, "model_{}.pth".format(str(iter_id).zfill(iter_str_len))
        )
        fp = open(log_file, "w")
        fp.write(f)
        fp.close()
        if iter_id != "final":
            return_id = os.system(
                "{} SOLVER.TEST_ITER {} SOLVER.MAX_ITER {}".format(
                    excute_cmd, iter_id + 2, iter_id + 2
                )
            )
        else:
