...
class ScaleMatchFactory(object):
    ...
    def create(cfg_SM):
        ...
        if cfg_SM.TYPE == "ScaleMatch":
            sm = ScaleMatch(
                anno_file=SM.TARGET_ANNO_FILE,
                bins=SM.BINS,
                except_rate=SM.EXCEPT_RATE,
                default_scale=SM.DEFAULT_SCALE,
                scale_range=SM.SCALE_RANGE,
                out_scale_deal=SM.OUT_SCALE_DEAL,
                mode=SM.SCALE_MODE,
                use_log_bins=SM.USE_LOG_SCALE_BIN,
            )
            ...
            sm = GaussianScaleMatch(
                ...
                SM.DEFAULT_SCALE,
                SM.OUT_SCALE_DEAL,
                SM.USE_LOG_SCALE_BIN,
                SM.SCALE_MODE,
                SM.GAUSSIAN_SAMPLE_FILE,
                SM.USE_MEAN_SIZE_IN_IMAGE,
                SM.MIN_SIZE,
            )
        else:
            raise ValueError(
                "cfg.DATALOADER.SCALE_MATCH.TYPE must be chose in ['ScaleMatch', 'MonotonicityScaleMatch'"
...
class ScaleMatch(object):
    ...
    def __init__(
        self,
        distribute=None,
        sizes=None,  # param group 1
        anno_file=None,
        bins=100,
        except_rate=-1.0,  # param group 2
        scale_range=(0.0, 2.0),
        default_scale=1.0,
        max_sample_try=5,
        out_scale_deal="clip",
        ...
        if anno_file is not None:
            if except_rate < 0:
                except_rate = 1.0 / bins * 2
            distribute, sizes = ScaleMatch._get_distribute(
                json.load(open(anno_file))["annotations"],
                bins,
                except_rate,
                use_log_bins,
            )
    ...
    @staticmethod
    def _get_distribute(
        annotations, bins=100, except_rate=0.1, use_log_bins=False, mu_sigma=(-1, -1)
    ):
        """
    ...
    def __call__(self, image, target):
        ...
        if len(target.bbox) == 0:
            ...
            if self.fail_time % 1 == 0:
                warnings.warn(
                    "Scale Matching failed for {} times, you may need to change the mean to min. "
                    "dst_size is {}, src_size is {}, sizes".format(
                        self.fail_time, dst_size, src_size, sizes
                    )
                )
...
class MonotonicityScaleMatch(object):
    def __init__(
        ...
        if except_rate < 0:
            except_rate = 1.0 / bins * 2
        dst_distri, dst_sizes = ScaleMatch._get_distribute(
            json.load(open(dst_anno_file))["annotations"],
            bins,
            except_rate,
            use_log_bins,
            mu_sigma,
        ...
        src_sizes = MonotonicityScaleMatch.match_distribute(
            json.load(open(src_anno_file))["annotations"], dst_distri_cumsum
        )
    ...
    def _sample_by_distribute(self, src_size):
        ...
        dst_size = (
            src_size - self.src_sizes[bin_i]
        ) / src_bin_d * dst_bin_d + self.dst_sizes[bin_i]
        return dst_size
...
class GaussianScaleMatch(MonotonicityScaleMatch):
    def __init__(
        ...
        use_log_bins=False,
        mode="bilinear",
        standard_gaussian_sample_file=None,
        use_size_in_image=True,
        min_size=0,
        debug_no_image_resize=False,
        debug_close_record=False,
    ):
        ...
        if except_rate < 0:
            except_rate = 1.0 / bins * 2
        mu, sigma = mu_sigma
        dst_distri, dst_sizes = GaussianScaleMatch._get_gaussain_distribute(
            mu,
            sigma,
            bins,
            except_rate,
            use_log_bins,
            standard_gaussian_sample_file,
            min_size,
        )
        dst_distri_cumsum = np.cumsum(dst_distri)
        src_sizes = GaussianScaleMatch.match_distribute(
            json.load(open(src_anno_file))["annotations"],
            dst_distri_cumsum,
            use_size_in_image,
        )
        self.src_sizes = src_sizes
    ...
    def _sample_by_distribute(self, src_size):
        ...
        dst_size = np.exp(
            (np.log(src_size) - np.log(self.src_sizes[bin_i])) / src_bin_d * dst_bin_d
            + np.log(self.dst_sizes[bin_i])
        )
        return dst_size
    ...
    def match_distribute(src_annotations, dst_distri_cumsum, use_size_in_image=True):
        def get_json_sizes(annos):
            ...
            if len(annos) > 0 and "ignore" in annos[0]:
                annos = [anno for anno in annos if not anno["ignore"]]
            sizes = np.sqrt(
                np.array([anno["bbox"][2] * anno["bbox"][3] for anno in annos])
            )
            sizes = sizes[sizes > 0]
            return sizes
...
class DebugScaleRecord(object):
    ...
    def __call__(self, scale):
        ...
        if (update and self.iters > 1000) or self.iters == 1000:
            warnings.warn(
                "update record scale {} -> {}".format(
                    last_record_scales, self.debug_record_scales
                )
            )
