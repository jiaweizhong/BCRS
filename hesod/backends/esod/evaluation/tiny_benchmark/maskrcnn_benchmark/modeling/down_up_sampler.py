...
class DeConvUpSampler(nn.Module):
    ...
    def __init__(
        ...
        if no_transform1:
            ...
            self.transform1 = self.build_transform(
                num_inputs, dim_reduced, dim_reduced, num_conv
            )
        self.deconv1 = ConvTranspose2d(dim_reduced, dim_reduced, 2, 2, 0)
        self.transform2 = self.build_transform(
            dim_reduced, dim_reduced, dim_reduced, num_conv
        )
        self.deconv2 = ConvTranspose2d(dim_reduced, num_inputs, 2, 2, 0)

        for modules in [
            self.transform1.modules(),
            self.transform2.modules(),
    ...
    def build_transform(self, in_channel, reduce_channel, out_channel, num_conv):
        ...
        transforms.append(
            CBRBlock(
                reduce_channel, out_channel, 3, 1, 1, self.no_relu, self.use_leaky_relu
            )
        )
    ...
    def forward(self, features):
        for i in range(len(features)):
            features[i] = F.interpolate(
                features[i], scale_factor=self.upsample_rate[i], mode=self.upsample_mode
            )
        return features
...
def build_sampler(cfg):
    ...
    assert (
        len(upsample_rate) == 0 or len(cfg.MODEL.FPN.UPSAMPLE_RATE) == 0
    ...
    if upsample_mode == "deconv":
        ...
        sampler = DeConvUpSampler(
            num_conv=upsample_transform_num_conv, no_transform1=True
        )
        ...
        sampler = DeConvUpSampler(
            num_conv=upsample_transform_num_conv, use_leaky_relu=True
        )
    elif upsample_mode == "downsample":
        downsample_rate = (
            torch.round(1 / torch.Tensor(upsample_rate))
            .numpy()
            .astype(np.int64)
            .tolist()
        )
