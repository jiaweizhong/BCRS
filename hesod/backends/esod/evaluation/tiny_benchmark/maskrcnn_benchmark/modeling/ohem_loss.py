...
class OHEMLoss(object):
    def __init__(self, neg_rate, binary_logits=False):
        ...
        self.cross_entropy = (
            F.binary_cross_entropy_with_logits if binary_logits else F.cross_entropy
        )
    ...
    def __call__(self, pred, label):
        ...
        pos_loss = self.cross_entropy(
            pos_pred, pos_label, size_average=False, reduce=False
        )
        neg_loss = self.cross_entropy(
            neg_pred, neg_label, size_average=False, reduce=False
        )

        num_neg = (
            num_pos * self.neg_rate
            if len(neg_pred) > num_pos * self.neg_rate
            else len(neg_pred)
        )
        num_neg = int(num_neg)
...
class OHEM2Loss(object):
    ...
    def __call__(self, pred, label):
        ...
        if len(pos_pred) > 0:
            ...
            pos_loss = torch.Tensor([0.0]).to(pred.device)
        ...
        if len(neg_loss) > max_num_neg:
            ...
            if num_hard > 0:
                hard_neg_loss = neg_loss[sort_idx[:num_hard].long()]
            if num_random > 0:
                ridx = (torch.randperm(int(len(neg_loss) - num_hard)) + num_hard)[
                    :num_random
                ]
                rand_neg_loss = neg_loss[sort_idx[ridx.long()].long()]
            if num_hard == 0:
                neg_loss = rand_neg_loss
            elif num_random == 0:
                neg_loss = hard_neg_loss
            else:
                neg_loss = torch.cat((hard_neg_loss, rand_neg_loss), 0)
