...
class LOCLossComputation(object):
    ...
    def __init__(self, cfg):
        ...
        if cls_loss_name == "fixed_focal_loss":
            self.cls_loss_func = FixSigmoidFocalLoss(
                cfg.MODEL.LOC.LOSS_GAMMA, cfg.MODEL.LOC.LOSS_ALPHA
            )
        ...
        self.centerness_weight_reg = (
            cfg.MODEL.LOC.TARGET_GENERATOR == "fcos"
            and cfg.MODEL.LOC.FCOS_CENTERNESS_WEIGHT_REG
        )
    ...
    def __call__(self, locations, box_cls, box_regression, centerness, targets):
        ...
        cls_losses = self.cls_loss_func(box_cls_flatten, labels_flatten)
        ...
        if pos_inds.numel() > 0:
            if self.centerness_weight_reg:
                reg_weights = centerness_targets = (
                    self.prepare_targets.compute_centerness_targets(reg_targets_flatten)
                )
            else:
                reg_weights = label_flatten_max[pos_inds]
            reg_loss = self.box_reg_loss_func(
                box_regression_flatten, reg_targets_flatten, reg_weights
            )
        else:
        ...
        if isinstance(cls_losses, (list,)):
            losses = {
                "loss_cls{}".format(i): cls_loss * self.cls_loss_weight
        ...
        if centerness is not None:
            centerness_flatten = [
                centerness[l].reshape(-1) for l in range(len(centerness))
            ]
...
class LabelMapShower(object):
    ...
    def __call__(self, labels, box_cls):
        ...
        for i, (label, cls) in enumerate(zip(labels, box_cls)):
            ...
            labels[i] = labels[i].reshape(
                (cls.shape[0], 1, cls.shape[-2], cls.shape[-1])
            )
