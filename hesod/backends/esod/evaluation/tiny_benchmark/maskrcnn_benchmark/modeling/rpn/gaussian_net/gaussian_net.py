...
class GauHead(torch.nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        init_modules = (
            [self.cls_tower, self.cls_logits, self.bbox_tower, self.gau_logits]
            if self.use_more_logits
...
class GauModule(torch.nn.Module):
        ...
        if self.training:
            ...
            if self.vis_infer:
                return self._forward_test(
                    self.loc_strides, logits, images.image_sizes, images, targets
                )
            else:
    ...
    def _forward_test(
        self, loc_strides, score_logits, image_sizes, images=None, targets=None
    ):
        boxes = self.box_selector_test(
            loc_strides, score_logits, image_sizes, images, targets
        )
        return boxes, {}
...
def eval_gt(self, locations, targets, images, cls_logits):
    ...
    _, _, matched_gt_idxs = self.loss_evaluator.reshape(
        labels, cls_logits, matched_gt_idxs
    )
