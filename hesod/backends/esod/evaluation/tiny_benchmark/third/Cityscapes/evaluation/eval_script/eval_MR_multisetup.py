...
class COCOeval:
    ...
    def __init__(
        ...
        assert filter_type in [
            "height",
            "size",
        ], "filter type must be 'height' or 'size'"
        ...
        for gt in gts:
            ...
            if "vis_ratio" not in gt:
                if "vis" not in gt:
                    ...
                    gt["vis_ratio"] = (gt["vis"][-1] / gt["bbox"][-1]) * (
                        gt["vis"][-2] / gt["bbox"][-2]
                    )
            if "size" not in gt:
            ...
            if self.filter_type == "size":
                gt["ignore"] = (
                    1
                    if (
                        gt["size"] < self.params.HtRng[id_setup][0]
                        or gt["size"] > self.params.HtRng[id_setup][1]
                    )
                    or (
    ...
    def evaluateImg(self, imgId, catId, hRng, vRng, maxDet):
        ...
        for idx, d in enumerate(dt):
            ...
            if self.filter_type == "size":
                ...
                if (
                    dsize >= hRng[0] / self.params.expFilter
                    and dsize < hRng[1] * self.params.expFilter
                ):
                    _dt.append(d)
                    dtind.append(idx)
            else:
                if (
                    d["height"] >= hRng[0] / self.params.expFilter
...
def get_fppi(min, max, n, EPS=1e-10):
    ...
    fppis = np.exp(
        np.arange(np.log(fppi_range[0]), np.log(fppi_range[1]) + EPS, log_step)
    )
...
class Params:
    ...
    def setDetParams(self):
        ...
        self.fppiThrs = get_fppi(0.01, 1.0, 9) / Params.CUT_WH[0] / Params.CUT_WH[1]  #
        #########################################################################################################
        self.maxDets = [1000]
        self.expFilter = 1.25
        self.useCats = 1
        ...
        if Params.CITYPERSON_STANDARD:
            self.iouThrs = (
                np.array([0.5, 0.75])
                if Params.IOU_THS is None
                else np.array(Params.IOU_THS)
