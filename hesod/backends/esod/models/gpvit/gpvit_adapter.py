...
class GPViTAdapter(GPViT):
    ...
    def forward(self, x):
        ...
        if self.add_vit_feature:
            ...
            x3 = F.interpolate(
                x2, scale_factor=0.5, mode="bilinear", align_corners=False
            )
            x4 = F.interpolate(
                x2, scale_factor=0.25, mode="bilinear", align_corners=False
            )
...
class GPViTAdapterSingleStageESOD(GPViTAdapter):
    ...
    def build_mask_indices(
        self, clusters: Optional[torch.Tensor], feat_size: Tuple[int, int]
    ):
        if clusters is None:
            return None, None
        ...
        mask_indices = (bi * H * W + (gy + y1) * W + (gx + x1)).view(
            -1
        )  # shape(n * w*h)
    ...
    def feat_slice(
        self, featmaps: List[torch.Tensor], clusters: torch.Tensor, scales: List[int]
    ):
        ...
        if getattr(self, "fs_grids", None) is None or self.fs_grids[0].size(1) != w * h:
            ...
            for s in scales:
                ...
                gxy = torch.stack(
                    (gy.reshape(-1), gx.reshape(-1)), dim=0
                )  # shape(2, w*h)
        ...
        for fi, s in enumerate(scales):
            ...
            H_, W_ = H // s, W // s
            mask_indices = (bi * H_ * W_ + (gj + t) * W_ + (gi + l)).view(
                -1
            )  # shape(n * w*h)

            fm = (
                featmaps[fi].flatten(2).transpose(1, 2).contiguous()
            )  # (B,C,H,W) -> (B,H*W,C)
    ...
    def feat_slice2(
        self,
        featembs: List[torch.Tensor],
        scales: List[int],
        clusters: torch.Tensor,
        mask_patch_resolution: List[int],
    ):
    ...
    def forward(self, x):
        ...
        B, C, H, W = x.shape
        mask_indices, cluster_size_ratio = self.build_mask_indices(
            clusters, (H // 8, W // 8)
        )
        ...
        if mask_indices is not None and True:
            ...
            deform_inputs1, deform_inputs2 = deform_inputs(
                torch.zeros((0, 0, H, W), device=x.device)
            )
            patch_resolution = (H // cluster_size_ratio, W // cluster_size_ratio)
            H, W = patch_resolution
            x, c2, c3, c4 = self.feat_slice2(
                [x, c2, c3, c4], [1, 1, 2, 4], clusters, patch_resolution
            )
            bs = x.size(0)
...
class InteractionBlock_GPViT(InteractionBlock):
    @staticmethod
    def chunk_feat(
        x: torch.Tensor, mask_indices, patch_resolution, cluster_size_ratio=8
    ):
    ...
    def forward(
        ...
        x0 = x
        x, patch_resolution = self.chunk_feat(
...
def deform_inputs(x):
    ...
    spatial_shapes = torch.as_tensor(
        [(h // 8, w // 8)], dtype=torch.long, device=x.device
    )
    # tensor([0])
    level_start_index = torch.cat(
        (spatial_shapes.new_zeros((1,)), spatial_shapes.prod(1).cumsum(0)[:-1])
