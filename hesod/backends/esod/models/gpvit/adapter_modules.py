...
class MSDeformAttn(nn.Module):
    ...
    @force_fp32(
        apply_to=("query", "reference_points", "input_flatten", "input_padding_mask")
    )
    def forward(
        self,
        query,
        ...
        output = MultiScaleDeformableAttnFunction.apply(
            ...
            input_level_start_index,
            sampling_locations,
            attention_weights.to(dtype=torch.float32),
            self.im2col_step,
        )
...
def get_reference_points(spatial_shapes, device):
    ...
    for lvl, (H_, W_) in enumerate(spatial_shapes):
        ...
        ref_y, ref_x = torch.meshgrid(
            torch.arange(0.5, H_, 1.0, dtype=torch.float32, device=device),
            torch.arange(0.5, W_, 1.0, dtype=torch.float32, device=device),
        )
