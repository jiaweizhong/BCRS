...
def darknet2uavdt():
    save_dir = join(
        opt.dataset, "UAV-benchmark-MOTD_v1.0", "RES_DET", "det_EfficientSOD"
    )
    test_videos = _readlines(join(opt.dataset, "split", "test_video.txt"))
    for video_name in tqdm(test_videos):
        detections = []
        img0 = cv2.imread(
            glob(join(opt.dataset, "UAV-benchmark-M", video_name, "*.jpg"))[0]
        )
        ...
        for path in sorted(glob(join(opt.pred, "%s_img*.txt" % video_name))):
            ...
            for cls, xc, yc, w, h, conf in bboxes:
                x1, y1 = (xc - w / 2.0) * width, (yc - h / 2.0) * height
                w, h = w * width, h * height
                detections.append(
                    "%d,-1,%.6f,%.6f,%.6f,%.6f,%.6f,1,-1\n"
...
def darknet2tinyperson():
    with open(
        join(opt.dataset, "mini_annotations", "tiny_set_test_all.json"), "r"
    ) as f:
        anno = json.load(f)
    image_id_dict = {item["file_name"]: item["id"] for item in anno["images"]}
    image_shape_dict = {
        item["id"]: [item["width"], item["height"]] for item in anno["images"]
    }
    ...
    for path in sorted(glob(join(opt.pred, "*.txt"))):
        ...
        v = (
            image_id_dict[k]
            if k in image_id_dict
            else image_id_dict["pure_bg_images/%s" % image_name]
        )
        ...
        for cls, xc, yc, w, h, conf in bboxes:
            ...
            if w * h > 30**2:  # for better performance on AP_tiny (~0.00x)
                continue
            detections.append(
                {"bbox": [x1, y1, w, h], "score": conf, "image_id": v, "category_id": 1}
            )
    with open(
        join(
            "evaluation",
