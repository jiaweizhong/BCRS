...
def convert_retinanet():
    ...
    features_retina = [
        features_retina[str(i)] for i in [2, 1, 0]
    ]  # large, medium, small
...
def convert_rtmdet():
    ...
    for k, v in model_state_dict.items():
        ...
        if "backbone.stem." in k:
            ...
            if m == 4 and n == 1:
                k = k.replace(
                    f"model.{2+csplayer_cnt}.conv", f"model.{2+csplayer_cnt}.cv"
                )
            ...
            try:
                m, n = list(
                    map(int, k.split("neck.top_down_layers.")[1].split(".")[:2])
                )
            except:
                m = int(k.split(".")[2])
                n = -1
            ...
            if n != -1:
                k = k.replace(
                    f"neck.top_down_layers.{m}.{n}.", f"model.{2+3+csplayer_cnt}."
                )
            else:
                k = k.replace(
                    f"neck.top_down_layers.{m}.", f"model.{2+3+2+csplayer_cnt}."
                )
        elif "neck.downsample_layers." in k:
            li = int(k.split("neck.downsample_layers.")[1].split(".")[0])
            k = k.replace(
                f"neck.downsample_layers.{li}.", f"model.{2+3+2+1+csplayer_cnt+3*li}."
            )
        elif "neck.bottom_up_layers." in k:
            li = int(k.split("neck.bottom_up_layers.")[1].split(".")[0])
            k = k.replace(
                f"neck.bottom_up_layers.{li}.", f"model.{2+3+2+1+2+csplayer_cnt+3*li}."
            )
        else:
            continue
...
def convert_gpvit():
    ...
    updated_state_dict.update(
        {k: v for k, v in model_dict.items() if k not in updated_state_dict}
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="retinanet",
        choices=["retinanet", "rtmdet", "gpvit"],
    )
    parser.add_argument(
        "--img_path",
        type=str,
        default="VisDrone/VisDrone2019-DET-train/images/0000002_00005_d_0000014_masked.jpg",
        help="directory for prediction results (*.txt)",
    )
