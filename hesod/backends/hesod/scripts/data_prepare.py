...
try:
    ...
    sam = sam_model_registry[model_type](checkpoint=sam_checkpoint).to(
        device
    )  # .half()  Warning: Precision Drops
...
def check_center(crop):
    ...
    s = 0.15
    return ((yc - h / 2.0).abs() > h * s) | ((xc - w / 2.0).abs() > w * s)


def segment_image(image, labels, width, height):
    if len(labels) == 0:
        return torch.zeros(image.shape[:2], dtype=torch.float16).to(device), np.full(
            (0,), False
        )
    ...
    if max(width, height) > 1024:
        ...
        for j in range(ny):
            for i in range(nx):
                grid = np.array(
                    [i / nx, j / ny, (i + 1) / nx, (j + 1) / ny], dtype=labels.dtype
                )
                indices = (
                    (grid[0] < x2) & (x1 < grid[2]) & (grid[1] < y2) & (y1 < grid[3])
                )
                if indices.sum() == 0:
                    continue

                x1_, y1_, x2_, y2_ = (
                    (x1[indices] - grid[0]).clip(0, 1 / nx),
                ...
                mask_k, invalid_k = segment_image(
                    image[y1_:y2_, x1_:x2_], labels_, width_, height_
                )
    ...
    for i, (x1, y1, x2, y2) in enumerate(input_boxes.cpu()):
        ...
        invalid[i] = (
            check_center(crop)
            | check_break(crop.sum(dim=1))
            | check_break(crop.sum(dim=0))
        )
...
def gen_mask(
    label_path, image, cls_ratio=False, thresh=0.5, sam_only=False, sam_mode="auto"
):
    ...
    if use_sam:
        ...
        if stride != 1:
            sam_res = F.interpolate(
                sam_res[None, None, ...].float(),
                size=(ny, nx),
                mode="bilinear",
                align_corners=False,
            )[0, 0]
    ...
    c, xc, yc, w, h = labels.T
    x1, y1, x2, y2 = (
        ((xc - w / 2.0) * nx).astype(np.int32).clip(0),
        ((yc - h / 2.0) * ny).astype(np.int32).clip(0),
        np.ceil((xc + w / 2.0) * nx).astype(np.int32).clip(0, nx),
        np.ceil((yc + h / 2.0) * ny).astype(np.int32).clip(0, ny),
    )
    input_boxes = np.stack((x1, y1, x2, y2), axis=1)
    ...
    for i, (x1, y1, x2, y2) in enumerate(input_boxes):
        ...
        r_size = max(area_min / area, 1.0) ** 2
        # elif area > area_max:
        ...
        curr_wt = np.zeros_like(masked_wt) + math.log(r) + 1.0
...
def prepare_visdrone():
    ...
    for sub_dir in glob(join(root, "VisDrone2019-DET-*")):
        ...
        for image_path in tqdm(images):
            ...
            height, width, _ = image.shape
            label_path = image_path.replace("images", "annotations").replace(
                ".jpg", ".txt"
            )
            ...
            for line in _readlines(label_path):
                if line[-1] == ",":
                    line = line[:-1]
                x1, y1, w, h, score, cls, truncation, occlusion = list(
                    map(int, line.split(","))
                )
                if cls in [0, 11]:
...
def prepare_uavdt():
    ...
    for i, path in enumerate(glob(join(label_dir, "*ignore.txt"))):
        ...
        for frameID, x1, y1, w, h, _ in tqdm(labels, desc="%02d/50" % (i + 1)):
            ...
            input_path = (
                ...
                else masked_path.replace("_masked.jpg", ".jpg")
            )
    ...
    data_split["train"], data_split["valid"] = (
        data_split["train"][sep:],
        data_split["train"][:sep],
    )
    ...
    for mode in ["train", "valid", "test"]:
        ...
        for video_name in tqdm(data_split[mode], desc=mode):
            ...
            ignores = np.loadtxt(
                ignore_path, usecols=(0, 2, 3, 4, 5), dtype=int, delimiter=","
            )
            ignore_dict = {}
            for frameID, x1, y1, w, h in ignores:
                xyxy = np.array([[x1, y1, x1 + w, y1 + h]])
                if frameID in ignore_dict:
                    ignore_dict[frameID] = np.concatenate(
                        (ignore_dict[frameID], xyxy), axis=0
                    )
                else:
                    ignore_dict[frameID] = xyxy

            labels = np.loadtxt(
                label_path, usecols=(0, 2, 3, 4, 5, 8), dtype=int, delimiter=","
            )
            label_dict = {}
            for frameID, x1, y1, w, h, cls in labels:
                ...
                if frameID in ignore_dict:
                    ...
                    if (
                        np.logical_and(
                            np.logical_and(
                                ignore_regions[:, 0] < xc, xc < ignore_regions[:, 2]
                            ),
                            np.logical_and(
                                ignore_regions[:, 1] < yc, yc < ignore_regions[:, 3]
                            ),
                        ).sum()
                        > 0
                    ):
...
def prepare_tinyperson():
    ...
    root = opt.dataset
    label_file_dict = {
        "train": join(root, "mini_annotations", "tiny_set_train_all_erase.json"),
    ...
    for mode in ["train", "test"]:
        ...
        for item in image_dict.values():
            ...
            if image is None:
                raise FileNotFoundError(
                    f"failed to read TinyPerson image: {image_path}"
                )
...
def prepare_seadronesseev2():
    ...
    for mode, ann_file in split_dict.items():
        ...
        for item in anno["annotations"]:
            ...
            width, height = image_dict[_id]["shape"]
            xc, yc = (x1 + w / 2.0) / width, (y1 + h / 2.0) / height
...
def prepare_seaperson():
    ...
    for mode, label_file in label_file_dict.items():
        ...
        missing_ids = [
            _id for _id, item in image_dict.items() if not exists(item["image_path"])
        ]
        if missing_ids:
            samples = "\n".join(
                f'  - {image_dict[_id]["image_path"]}' for _id in missing_ids[:5]
            )
        ...
        for item in anno["annotations"]:
            ...
            if x2c - x1c < 1 or y2c - y1c < 1:
                ...
                continue
            cls = cat_id_to_yolo[item["category_id"]]
            xc, yc = (x1c + x2c) / 2.0 / width, (y1c + y2c) / 2.0 / height
        ...
        if n_degenerate:
            print(
                f"seaperson/{mode}: skipped {n_degenerate} zero-area/out-of-frame annotation(s)"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="VisDrone",
        help="dataset, e.g., VisDrone, UAVDT, TinyPerson, and SeaDronesSeeV2",
    )
    parser.add_argument(
