...
def plot_one_box_PIL(box, im, color=(128, 128, 128), label=None, line_thickness=None):
    ...
    if label:
        ...
        draw.rectangle(
            [box[0], box[1] - txt_height + 4, box[0] + txt_width, box[1]], fill=color
        )
        draw.text(
...
def plot_results(start=0, stop=0, bucket="", id=(), labels=(), save_dir=""):
    ...
    for fi, f in enumerate(files):
        try:
            # results = np.loadtxt(f, usecols=[2, 3, 4, 8, 9, 12, 13, 14, 10, 11], ndmin=2).T
            results = np.loadtxt(
                f, usecols=[2, 3, 4, 11, 12, 17, 18, 19, 13, 14], ndmin=2
            ).T
            n = results.shape[1]  # number of rows
            x = range(start, min(stop, n) if stop else n)
            for i in range(10):
...
def plot_cluster(torch_imgs, clusters):
    ...
    imgs = (imgs[..., ::-1] * 255.0).astype(np.uint8)
...
class LatencyBucket(object):
    ...
    def load(self, file_path):
        ...
        self.params = data.get(
            "params"
        )  # absent in buckets.json saved before this field existed
    ...
    def plot(self):
        ...
        nums, gflops, latency = (
            np.array(self.nums),
            np.array(self.gflops),
            np.array(self.latency),
        )
        total_nums = 8 * 5
        ...
        ax2 = ax.twinx()
        (line,) = ax2.plot(
            xrange, bucket_latency, color="#ff7f0e", marker="o", label="Latency"
        )
