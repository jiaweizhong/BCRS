...
def detect(opt):
    ...
    save_img = not opt.nosave and (
        not source.endswith(".txt") or True
    )  # save inference images
    webcam = (
        source.isnumeric()
        or (source.endswith(".txt") and False)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if opt.view_center:
            masks = (
                (masks == F.max_pool2d(masks, 3, stride=1, padding=1)) & (masks > 0.3)
            ).float()
        clusters = p_det[1][0]
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if opt.view_cluster:
                ...
                heatmap = (masks[i, 0].cpu().numpy() * 255.0).astype(np.uint8)
                heatmap = cv2.resize(
                    heatmap, (im0.shape[1], im0.shape[0]), cv2.INTER_CUBIC
                )
                ...
                if osp.exists(label_path):
                    ...
                    for ci, xc, yc, w, h in gt_bboxes:
                        ...
                        label = (
                            None
                            if opt.hide_labels
                            else (names[c] if opt.hide_conf else f"{names[c]}")
                        )
                        xyxy = [
                            (xc - w / 2.0) * im0.shape[1],
                            (yc - h / 2.0) * im0.shape[0],
                            (xc + w / 2.0) * im0.shape[1],
                            (yc + h / 2.0) * im0.shape[0],
                        ]
                        plot_one_box(
                    ...
                    if os.path.exists(gt_mask_path):
                        ...
                        heatmap = (gt_mask * 255.0).astype(np.uint8)
                ...
                for ci, xyxy in enumerate(cluster):
                    ...
                    plot_one_box(
                        ...
                        im2,
                        color=(0, 255, 0),
                        line_thickness=opt.line_thickness * 2,
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--view-cluster", action="store_true", help="visualize clusters"
    )
    parser.add_argument(
        "--view-center", action="store_true", help="visualize heatmap centers"
    )
