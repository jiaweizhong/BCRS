...
def test(
    ...
    if not training:
        if device.type != "cpu" and False:
            model(
                torch.zeros(1, 3, imgsz // 2, imgsz // 2)
                .to(device)
    ...
    for batch_i, (img, targets, masks, m_weights, paths, shapes) in enumerate(
        ...
        (out, p_det), p_seg = model(
            (img, [masks]) if use_gt else img, augment=augment
        ...
        if compute_loss:
            ...
            loss += compute_loss(
                train_out, targets, imgsz=img.shape, masks=masks, m_weights=m_weights
            )[1][
                :6
            ]  # box, obj, cls, pixl, area, dist
        ...
        if isinstance(p_det, tuple):
            clusters = (
                p_det[1][0] if p_det is not None else torch.zeros((0, 5), device=device)
            )
            clusters = [clusters[clusters[:, 0] == bi, 1:] for bi in range(nb)]
            statistic_items += cluster_recall(
                clusters, targets, imgsz=(width, height), mode="bbox"
            )
            if not training and opt.task == "measure":
            ...
            if save_regions:
                ...
                for si in range(nb):
                    ...
                    selected_regions[image_id] = (
                        clusters[si].detach().cpu().round().to(torch.int32).tolist()
                    )
        lb = (
        ...
        for si, pred in enumerate(out):
            ...
            if save_txt:
                ...
                for *xyxy, conf, cls in predn.tolist():
                    ...
                    filename = (
                        (path.stem + ".txt")
                        if "uavdt" not in opt.data
                        else (path.parent.stem + "_" + path.stem + ".txt")
                    )
    ...
    if len(stats):
        ...
        nt = torch.zeros(1)
    bpr, occupy = statistic_items[0] / nt.sum(), (statistic_items[1] + 1e-6) / (
        statistic_items[2] + 1e-6
    )
    if hm_metric:
    ...
    if not training:
        if opt.task == "measure":
            ...
            lbucket.params = n_p
            print(
                "GFLOPs: %.1f. Params: %d. FPS: %.1f "
                % (np.mean(gflops), n_p, 1000.0 / t[0]),
                end="",
            )
        print(
    ...
    if save_json and len(jdict):
        ...
        pred_json = str(
            save_dir / f"{w}_predictions.json"
        )  # predictions json (raw, 0-indexed category_id)
        print("\nSaving %s..." % pred_json)
        with open(pred_json, "w") as f:
            json.dump(jdict, f)

        # HESOD local patch: upstream resolved anno_json (and for TinyPerson, called
        # format_tinyperson()) BEFORE writing pred_json, so an unimplemented dataset
        # branch (UAVDT: bare `raise NotImplementedError`) or a hardcoded annotations
        # path that doesn't exist on this machine crashed test.py before any
        # predictions were ever saved to disk. pred_json now always exists once
        # save_json is set, for every dataset; the optional pycocotools comparison
        # below stays best-effort inside the pre-existing try/except. Note
        # format_tinyperson() mutates jdict in-memory only at this point, so it no
        # longer affects the on-disk pred_json (which downstream tooling, e.g.
        # BCRS/tools/audit_failure_cases.py, consumes in raw ESOD format).
        try:  # https://github.com/cocodataset/cocoapi/blob/master/PythonAPI/pycocoEvalDemo.ipynb
            ...
            if "visdrone" in opt.data.lower():
                anno_json = (
                    "./datasets/VisDrone/annotations/val.json"  # annotations json
                )
                ...
                anno_json = format_seadronesseev2(
                    jdict,
                    gt_json="/root/autodl-tmp/CompressedVersion/annotations/instances_val.json",
                )
            else:
                raise NotImplementedError(
                    f"Ground-Truth file for {os.path.basename(opt.data)} not found."
                )
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
    return (
        (mp, mr, map50, map, bpr, occupy, *(loss.cpu() / len(dataloader)).tolist()),
        maps,
        t,
    )
...
def format_seadronesseev2(
    ...
    with open(gt_json, "r") as f:
        anno = json.load(f)
    image_id_dict = {
        os.path.splitext(item["file_name"])[0]: item["id"] for item in anno["images"]
    }
    ...
    patched_gt = os.path.join(
        os.path.dirname(gt_json), ".instances_val_iscrowd_patched.json"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sparse-head", action="store_true", help="use sparse detection head"
    )
    parser.add_argument(
        "--hm-metric",
        action="store_true",
