...
**Status (2026-08-30):** Arms (1)-(7) are freshly trained and audited this session, cross-verified against raw `audit_buckets.py`/`vt_diagnose.py`/`measure.log` output. Arms (3) Spectral-only and (5) Concat-only both had crash histories (a DataLoader-worker memory leak, mitigated with `--workers 2`) and have since been cleanly retrained end-to-end; both reruns changed the numbers meaningfully (direction held for arm 3, arm 5's own story changed substantially -- see §3.4 point 2). One open item remains: arm (8) HESOD (Full)'s checkpoint predates this session's re-verification work entirely (2026-08-21, vs. every other arm's 2026-08-25 or later) and still needs its own rerun -- its mAP/recall/Params below carry over from that older run; BPR/GFLOPs/FPS were never measured for it on UAVDT at all.
