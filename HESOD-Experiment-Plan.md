...
**Status (2026-08-25): full 8-arm roster running from a clean queue (`scripts/esod_baseline/run_uavdt.sh`).** Arm (1) R0 is kept at its independently-audited rerun value (`audit_buckets.py`/`vt_diagnose.py` against an intact `best_predictions.json`, verified 2026-08-25) and will not be regenerated by this batch -- `run_uavdt.sh`'s skip-gate reuses its existing checkpoint/predictions rather than retraining. Arms (2)-(8), including HESOD (Full), are pending this run. See §6 for the runner script.
...
| **HESOD (Full, Ours)** | pending | pending | pending | pending | pending | pending | pending | pending | pending |
| **Delta ($\Delta$)** | pending | pending | pending | pending | pending | pending | pending | pending | pending |

- Known open question once results land: UAVDT has shown a large run-to-run mAP swing between independent R0 runs (~4.1pp mAP@.5 in the most recent pair) -- larger than SeaPerson's own measured noise floor (~0.4pp). Compare this batch's R0 against both the paper and the prior run before treating any single-run delta here as a confirmed finding.
...
- $^\dagger$ BPR is captured by the same `test.py --task val` pass SeaPerson's §4.2 BPR column comes from -- no UAVDT-specific gap, it just wasn't recovered from this rerun's log (the only BPR value found in raw logs, 0.866, came from a stale pre-rerun log, not this run). Recoverable without a full eval+measure redo by recomputing directly against the existing `best_predictions.json` still on the remote box (untouched -- the skip-gate in `run_uavdt.sh` depends on its existing) plus GT labels, once a small standalone script exists for it.
