...
### 4.3 Roster arms — in progress

Two single-variable arms queued on the same corrected recipe (`scripts/esod_baseline/run_tinyperson_scratch_hyp.sh`), not yet complete: channel-pooled dual-evidence concat (selector-side coverage loss, same recipe as §2's VisDrone arm) and box-regression size-weighting (`--box-loss size_weighted`, head-side, targets §1.6's #1 VisDrone gap item — kept as its own arm so it's never confounded with the selector-side change). Results pending.
...
1. **TinyPerson: channel-pooled-concat and box-regression size-weighting arms** (§4.3) — baseline (scratch hyp) is done and confirmed the gap-closing hyp-file fix; the other two arms from `scripts/esod_baseline/run_tinyperson_scratch_hyp.sh` are still running.
