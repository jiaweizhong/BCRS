...
**Status (2026-08-29):** Arms (1)-(4) and (6)-(7) are freshly trained and audited this session, cross-verified against raw `audit_buckets.py`/`vt_diagnose.py`/`measure.log` output. Arm (3) Spectral-only's original run crashed and resumed three times (a DataLoader-worker memory leak, since mitigated with `--workers 2`) and has now been cleanly retrained end-to-end -- the numbers below are that confirmed rerun (mAP@.5 moved 0.419$\to$0.396, Total Recall 90.38%$\to$88.83%; direction unchanged, magnitude corrected down). Two open items remain:
- Arm (5) Concat-only had the same crash history and is still being rerun to confirm its numbers aren't an artifact (`ARMS=uavdt_yolov5m_channel_pooled_concat`) -- because arm (4) Spectral-only (Pooled)'s own interpretation in §3.4 point 2 compares against arm (3)'s *old* number, that comparison is being held for rewrite until arm (5) also lands, rather than partially updating with a mix of confirmed and unconfirmed figures.
- Arm (8) HESOD (Full)'s checkpoint predates this session's re-verification work entirely (2026-08-21, vs. every other arm's 2026-08-25 or later) and still needs its own rerun -- its mAP/recall/Params below carry over from that older run; BPR/GFLOPs/FPS were never measured for it on UAVDT at all.
...
| **HESOD (Full, Ours)** | 0.371 | 0.195 | **90.17%** | **90.56%** | **84.88%** | **76.28%** | pending | 26.01 | pending |
| **Delta ($\Delta$)** | -1.4 pp | -1.9 pp | **+5.0 pp** | **+4.95 pp** | **+3.83 pp** | **+8.61 pp** | -- | -27.5% | -- |

R0's own reproduction is close to the paper (mAP@.5/.95: 0.385/0.214 vs. paper's 0.407/0.225, within ~1-2pp) but UAVDT has shown real run-to-run noise on this metric (~4pp mAP@.5 swing between independent R0 runs) -- larger than the mAP delta HESOD shows over R0 here. Combined with HESOD (Full)'s own stale checkpoint (above), **the mAP comparison in this table is unresolved**; the recall margin (~5pp, several times the observed noise floor) is on firmer footing. Both open items get resolved by the same two reruns noted above.
...
| **(5)** | **Concat-only** | $\mathcal{L}_{\mathrm{cover}}$ | CIoU | Coupled | 0.361 | 0.182 | 0.947 | 97.3 | 35.85 | 98.9 |
...
| **(8)** | **HESOD (Full)** | $\mathcal{L}_{\mathrm{cover}}$ | SABL | ISPPHead | 0.371 | 0.195 | pending$^\ddagger$ | pending$^\ddagger$ | 26.01 | pending$^\ddagger$ |
...
- $^\dagger$ Arm (1)'s BPR wasn't recovered from this rerun's log (a duplicate eval attempt overwrote it before this row could be re-audited) -- every other figure in this row was independently re-verified 2026-08-25 against an intact `best_predictions.json`/`measure.log`.
- $^\ddagger$ Arm (8) predates this session (§ Status above) and was never measured for BPR/GFLOPs/FPS on UAVDT at all.
...
| **(5) Concat-only** | 85.33% | 90.20% | 98.05% | 63.63% | 90.52% | 90.83% | 84.50% | 81.25% |
...
| **(8) HESOD (Full)** | n/a$^\S$ | n/a$^\S$ | n/a$^\S$ | n/a$^\S$ | 90.17% | 90.56% | 84.88% | 76.28% |

$^\S$ Size-bucket breakdown was never captured for arm (8)'s original (pre-session) run, only total + per-class recall.
...
2. **Total Recall and mAP diverge as the candidate pool gets denser -- read them together, not standalone**. Recall and mAP answer different questions: Total Recall (`audit_buckets.py`, conf$\ge$0.001, §1.2) only asks "is *some* box within IoU$\ge$0.5 of this GT," blind to how many *other* boxes are also present; mAP asks how precise the confidence-ranked prediction list is at every operating point, so it directly penalizes a denser but noisier candidate pool. Arm (5) Concat-only is the clearest case: it produces **3.18$\times$ R0's raw prediction count** (7.49M vs. 2.35M rows in `best_predictions.json`, for the same 373,997 GT) and every per-class/per-bucket recall number is higher than R0's -- yet mAP@.5 is **2.4pp lower** (0.361 vs. 0.385) and mAP@.5:.95 3.2pp lower (0.182 vs. 0.214). The ~5.1M "extra" candidates beyond R0's count are overwhelmingly false positives or redundant/imprecise duplicates that never help recall but do inflate the false-positive count driving down precision at most confidence thresholds. `vt_diagnose.py` corroborates the localization side specifically: arm (5)'s `right_class_low_iou` miss-share (55.2%, the highest of any arm) means many of its "recalled" boxes clear the lenient IoU$\ge$0.5 recall bar without clearing the much tighter IoU up to 0.95 that mAP@.5:.95 averages over -- hurting that metric even more than mAP@.5. Arm (4) Spectral-only (Pooled) versus arm (3)'s now-confirmed rerun tells a *different* story than it first appeared to: +3.10pp Total Recall (91.93% vs. 88.83%, more raw predictions: 5.94M vs. 5.14M) but mAP@.5 is now essentially flat (0.394 vs. 0.396, -0.2pp -- previously computed as -2.5pp against arm (3)'s uncorrected number). This is much closer to SeaPerson's own "no difference" result from its pooled-vs-unpooled confound-check (§4.2 note) than first thought -- UAVDT partially replicates it: mAP is a wash like SeaPerson, but Total Recall is not (SeaPerson's own pooled/unpooled recall also landed within 0.02pp of each other, 87.77% both; UAVDT's gap is >150$\times$ larger). `vt_diagnose.py` still shows the same directional shift toward less-precise routing under pooling (localization-failure share 45.1%$\to$54.6%, selector-dropped share 37.2%$\to$27.0%), just not enough to move mAP outside noise this time.
3. **SABL and ISPPHead diverge from their SeaPerson analogs in opposite directions**. SeaPerson found SABL raises Very Tiny recall (+0.67pp, §4.4.4); on UAVDT (arm 6 vs. arm 5, SABL's only clean isolated effect -- identical architecture/Params) it **drops** Very Tiny recall (-1.8pp) and Total Recall (-0.8pp) -- SABL's accuracy effect isn't a reliable win on either dataset, and UAVDT adds a domain where the direction actively reverses. ISPPHead, by contrast, replicates cleanly: arm (7) vs. arm (5) cuts GFLOPs 15.1% and Params 27.5% (SeaPerson: 22.6%/27.6%, §4.4.3) while mAP@.5 and mAP@.5:.95 both *improve* +1.0pp -- the closest UAVDT arm-for-arm match to a SeaPerson headline finding in either direction or magnitude this session.
