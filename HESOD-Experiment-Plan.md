...
**Status.** VisDrone: baseline reproduced close to the paper's own Gaussian-only ablation (§1), two roster arms (dual-evidence concat, channel-pooled concat) tested and both negative including under a hard patch-budget sweep (§2-3); SAM hybrid-mask arm queued. TinyPerson: baseline retrained on the corrected hyp file, gap closed by more than half (§4.1-4.2); channel-pooled-concat is a genuine positive result (first roster arm anywhere in this project to move AP, not just recall ceiling), box-regression size-weighting and `ratio=16` are both negative (§4.2-4.3). UAVDT: re-fetched from the official source, nc=1 baseline trained but suspiciously far above the paper's own number (~1.8-2.3x, eval-protocol mismatch confirmed real but not sufficient to explain it) — `nc=3` re-test queued to check whether the paper's own protocol is actually 3-class (§5).
...
**Eval-protocol investigation (in progress, not resolved).** Checked whether UAVDT's vendored MATLAB toolkit (`evaluation/UAV-benchmark-MOTD_v1.0/`) explains the gap: `utils/evalRes.m` defaults to a **single IoU=0.70 threshold** (not COCO's 0.5:0.95 average) with VOC-style `VOCap.m` AP integration. Built `scripts/esod_baseline/eval_at_iou_thresholds.py` (reuses `audit_buckets.py`'s YOLO loaders, real pycocotools `COCOeval`, prints AP at every individual IoU threshold) to test this directly: our AP at IoU=0.70 specifically is 52.2% — real movement in the right direction (vs. 40.9% averaged), but still ~2.3x the paper's 22.5%, so this does not fully explain it. `VOCap.m`'s AP integration (all-point PR envelope) is not meaningfully different from pycocotools' 101-point method, ruled out as a factor. **Current leading hypothesis:** the other baselines Table I compares ESOD against (FasterRCNN 11.0/23.4, ClusDet 13.7/26.5, DMNet 14.7/24.6, CDMNet 16.8/29.1, CEASC 17.1/30.9) are well-known numbers from the standard **3-class** UAVDT-DET literature — if the whole table uses that protocol for a fair comparison, `prepare_uavdt()`'s single-class collapse in the *public* code may not match what actually produced the paper's own Table I number. Added `--keep-classes` to `scripts/data_prepare.py` (all three trees) and `models/cfg/esod/uavdt_yolov5m_nc3.yaml` to test this directly (`UAVDT_v3`, training queued via `scripts/esod_baseline/run_overnight_sam_then_uavdt_nc3.sh`). **Do not treat the nc=1 numbers above as verified-comparable to the paper until this resolves.**
...
1. **UAVDT `nc=3` re-test** (§5) — training queued (`run_overnight_sam_then_uavdt_nc3.sh`, chained after the VisDrone SAM run below) to test whether the paper's Table I UAVDT number reflects the standard 3-class protocol rather than `prepare_uavdt()`'s single-class default. Until this resolves, report the nc=1 numbers as unverified-vs-paper.
2. **VisDrone + SAM hybrid masks** — checkpoint downloaded, `segment-anything` installed (via a site-packages symlink, `pip install -e .` never worked under this environment's Python 3.12/setuptools combination), a real upstream bug in `gen_mask()` found and fixed (CUDA tensor passed directly to `.numpy()`, ESOD-Baseline-Patches.md #8). Training queued in the same overnight run, `run_visdrone_sam.sh`.
3. Once UAVDT's protocol question is resolved and trusted, decide whether to test the roster arms there too (§1.6/§2's VisDrone conclusion and §4.3's TinyPerson conclusion actively disagree on which lever — selector-side vs. head-side — helps, so a third data point matters) and whether TinyPerson's "budget-constrained selector benefits more from dual-evidence fusion" hypothesis (§4.3) replicates on UAVDT.
