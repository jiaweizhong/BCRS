...
class EsodAdapter(BackendAdapter):
    ...
    def build(self, stage: str, experiment: ExperimentConfig) -> CommandSpec:
        ...
        argv = [experiment.python, str(experiment.backend_root / ("train.py" if stage == "train" else "test.py"))]

        if stage == "train":
            argv += [
                "--data", str(data_config),
                "--cfg", str(experiment.model_config_for(stage)),
                "--epochs", str(section.get("epochs", 50)),
                "--batch-size", str(section.get("batch_size", 8)),
                "--img-size", str(section.get("image_size", 1536)),
                "--workers", str(experiment.runtime.get("workers", 8)),
                "--device", experiment.devices,
            ...
            if hyp is not None:
                argv += ["--hyp", str(hyp)]
            for flag, option in (("hm_only", "--hm-only"), ("hm_metric", "--hm-metric")):
            ...
            argv += [
                ...
                "--batch-size", str(section.get("batch_size", 1)),
                "--img-size", str(section.get("image_size", 1536)),
                "--device", experiment.devices,
                "--task", str(section.get("task", "val")),
                "--project", str(output_dir.parent),
                "--name", output_dir.name + "_test",
                "--exist-ok",
            ]
            for flag, option in (
    ...
    def diagnostics(self, stage: str, experiment: ExperimentConfig) -> list[Diagnostic]:
        ...
        entrypoint = experiment.backend_root / ("train.py" if stage == "train" else "test.py")
        checks.append(self._path_diagnostic(f"{stage} entrypoint", entrypoint))
        section = experiment.train if stage == "train" else experiment.test
        for role in (("train", "val") if stage == "train" else ("val",)):
            checks.append(self._path_diagnostic(f"dataset {role}", self._split_input(experiment, role)))
    ...
    def _materialize_dataset(self, experiment: ExperimentConfig) -> Path:
        output = experiment.project_root / ".bcrs" / "generated" / experiment.name / "dataset.esod.yaml"
        output.parent.mkdir(parents=True, exist_ok=True)
        test_role = "test" if "test" in experiment.dataset.splits or "test" in experiment.dataset.adapter_options("esod").get("splits", {}) else "val"
        payload = {
