...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            img, msk, ratio, pad = letterbox(
                img, shape, msk=msk, auto=False, scaleup=self.augment
            )
        ...
        if self.augment:
            # Augment imagespace
            if not mosaic:
                img, msk, labels = random_perspective(
                    img,
                    msk,
                    labels,
                    degrees=hyp["degrees"],
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        mask_path = path.replace("/images/", "/masks/").replace(
            ".jpg", ".npy"
        )  # .replace('_masked.npy', '.npy')
        ...
        if r != 1:  # if sizes are not equal
            ...
            img = cv2.resize(
                img, (int(w0 * r), int(h0 * r)), interpolation=interpolation
            )
            if mask is not None:
                assert tuple(mask.shape[:2]) == (h0, w0)
                mask = cv2.resize(
                    mask, (int(w0 * r), int(h0 * r)), interpolation=interpolation
                )
...
def norm_imgs(imgs: torch.Tensor, model):
    ...
    if "yolo" in cfg_path:
        ...
        raise NotImplementedError(
            f"Please manually configure image normalization for {cfg_path}"
        )
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if i == 0:  # top left
            ...
            msk4 = np.full(
                (s * 2, s * 2, msk.shape[2]), 0.0, dtype=msk.dtype
    ...
    # Augment
    img4, msk4, labels4 = random_perspective(
        img4,
        msk4,
        labels4,
        segments4,
...
def letterbox(
    ...
    if shape[::-1] != new_unpad:  # resize
        ...
        msk = (
            cv2.resize(msk, new_unpad, interpolation=cv2.INTER_LINEAR)
            if msk is not None
            else None
        )
    ...
    if msk is not None:
        msk = cv2.copyMakeBorder(
            msk, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(0.0, 1.0)
        )  # add border
        ...
        if perspective:
            ...
            if msk is not None:
                msk = cv2.warpPerspective(
                    msk, M, dsize=(width, height), borderValue=(0.0, 1.0)
                )
        else:  # affine
            img = cv2.warpAffine(
            ...
            if msk is not None:
                msk = cv2.warpAffine(
                    msk, M[:2], dsize=(width, height), borderValue=(0.0, 1.0)
                )
...
def dataset_stats(path="data/coco128.yaml", verbose=False):
    ...
    for split in "train", "val", "test":
        ...
        x = []
        dataset = LoadImagesAndLabels(
            data[split], augment=False, rect=True
        )  # load dataset
        for label in tqdm(dataset.labels, total=dataset.n, desc="Statistics"):
            x.append(np.bincount(label[:, 0].astype(int), minlength=nc))
        x = np.array(x)  # shape(128x80)
        stats[split] = {
            "instances": {"total": int(x.sum()), "per_class": x.sum(0).tolist()},
            "images": {
                "total": dataset.n,
