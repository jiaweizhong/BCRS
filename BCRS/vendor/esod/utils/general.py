...
def check_mask(images, masks, center=False):
    import cv2

    images = images.permute(0, 2, 3, 1).contiguous().cpu().numpy().clip(0.0, 1.0)
    if center:
        maxima = F.max_pool2d(masks, 3, stride=1, padding=1) == masks
        masks *= maxima.type_as(masks)
    masks = masks[:, 0, :, :].cpu().numpy().clip(0.0, 1.0)
    for bi, img in enumerate(images):
        img = (img * 255.0).astype(np.uint8)
        msk = (masks[bi] * 255.0).astype(np.uint8)
        # if msk.shape[:2] != img.shape[:2]:
        msk = cv2.resize(msk, img.shape[:2][::-1], interpolation=cv2.INTER_CUBIC)

        heatmap = cv2.applyColorMap(msk, cv2.COLORMAP_RAINBOW)
        heatmap = cv2.cvtColor(heatmap, cv2.COLOR_RGB2BGR)
        merged = cv2.addWeighted(img, 0.4, heatmap, 0.5, 0)
        cv2.imwrite(f"tmp/{bi:02d}.jpg", merged)
...
def check_file(file):
    ...
    if Path(file).is_file() or file == "":  # exists
        ...
        url, file = (
            file,
            Path(urllib.parse.unquote(str(file))).name,
        )  # url, file (decode '%2F' to '/' etc.)
        file = file.split("?")[
            0
...
def labels_to_image_weights(labels, nc=80, class_weights=np.ones(80)):
    ...
    class_counts = np.array(
        [np.bincount(x[:, 0].astype(np.int64), minlength=nc) for x in labels]
    )
...
def box_ios(box1, box2, eps=1e-9):
    """
    Return intersection-over-self (Jaccard index) of boxes.
    Both sets of boxes are expected to be in (x1, y1, x2, y2) format.
    Arguments:
        box1 (Tensor[N, 4])
        box2 (Tensor[M, 4])
    Returns:
        ios (Tensor[N, M]): the NxM matrix containing the pairwise
                            ios values for every element in boxes1 and boxes2
    """
    ...
    return inter / (area1[:, None] + eps)  # ios = inter / area1
...
def non_max_suppression(
    ...
    # print(conf_thres, prediction.shape, torch.prod(torch.tensor(prediction.shape[:2])), xc.sum())
...
def gaussian2D(shape, sigma=1.0, thresh=None):
    ...
    if thresh is None and sigma is not None:
        ...
        h *= 1.0 / h.max()  # in case even number
...
def target2mask(targets, shape, nc, stride=8, thresh=0.3):
    cls_ratio = [
        1.83,
        5.35,
        13.82,
    ...
    single_cls = nc == 1
    mask = np.zeros((nc, ny, nx), dtype=np.float32)
    weight = np.ones((nc, ny, nx), dtype=np.float32)

    gain = torch.tensor([[1, 1, nx, ny, nx, ny]], dtype=dtype, device=device)
    for _, ci, xc, yc, w, h in targets * gain:
        ci = 0 if single_cls else ci.long().item()
        x1, x2, y1, y2 = (
            (xc - w / 2.0).long().clamp(0).item(),
            (xc + w / 2.0).ceil().long().clamp(1, nx).item(),
            (yc - h / 2.0).long().clamp(0).item(),
            (yc + h / 2.0).ceil().long().clamp(1, ny).item(),
        )
        gaussian = gaussian2D((y2 - y1, x2 - x1), sigma=None, thresh=thresh)
        # gaussian[gaussian >= thresh] = 1.0
        # gaussian = np.ones(1)
        # # assert gaussian.max() > thresh
        masked_hm = mask[ci, y1:y2, x1:x2]
        np.maximum(masked_hm, gaussian, out=masked_hm)

        # # r_size = max((6 * 6) / (w * h), (w * h) / (6 * 6))
        r_size = 1.0
        area_min, area_max = 4 * 4, 6 * 6
        area = (w * h).item()
        if area < area_min:
            r_size = area_min / area
        # elif area > area_max:
        #     r_size = area / area_max
        r_cls = cls_ratio[ci] * 10

        r = max(r_size, r_cls)
        masked_wt = weight[ci, y1:y2, x1:x2]
        np.maximum(masked_wt, math.log(r) + 1, out=masked_wt)

    return np.clip(mask, 0, 1, out=mask), weight
