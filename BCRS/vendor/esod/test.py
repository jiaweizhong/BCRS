...
def test(
    ...
    print(
        ...
        f"mAP@0.5: {map50:.4f} | Patch BPR (bpr): {bpr:.4f} | Heatmap BPR (hm_bpr): {hm_bpr:.4f}"
    )
    ...
    if save_json and len(jdict):
        ...
        data_str = str(opt.data).lower()
        if "visdrone" in data_str:
            cand_paths = [
                Path(opt.data).parent / "annotations" / "val.json",
                Path("./datasets/VisDrone/annotations/val.json"),
                Path("/root/autodl-tmp/VisDrone/annotations/val.json"),
            ]
        ...
        print("\nSaving COCO predictions JSON to %s..." % pred_json)
        with open(pred_json, "w") as f:
            json.dump(jdict, f)

        if anno_json and os.path.exists(anno_json):
