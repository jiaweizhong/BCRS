...
def permute_all_to_NHWA_K_not_concat(box_cls, box_delta, num_classes=80):
    box_cls_flattened = [
        permute_to_N_HWA_K(x, num_classes).reshape(-1, num_classes) for x in box_cls
    ]
...
class RetinaNetQueryDet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.CUSTOM.HEAD_BN:
            self.det_head = dh.RetinaNetHead_3x3_MergeBN(
                cfg, 256, 256, 4, self.anchor_num
            )
        ...
        self.qInfer = qf.QueryInfer(
            9, self.num_classes, self.query_threshold, self.query_context
        )
        ...
        self.query_anchor_generator = AnchorGeneratorWithCenter(
            sizes=[128],
            aspect_ratios=[1.0],
    ...
    def train_forward(self, batched_inputs, just_forward=False):
        ...
        losses = self.det_loss(
            gt_classes, gt_reg_targets, det_cls, det_delta, all_anchors
        )

        # query loss
        gt_query = self.get_query_gt(query_centers, gt_instances)
        query_forgrounds = [gt.sum().item() for gt in gt_query]
        _query_loss = self.query_loss(
            gt_query, query_logits, self.query_loss_gammas, self.query_loss_weights
        )
    ...
    def test_forward(self, images):
        ...
        if not self.query_infer:
            ...
            det_cls_query = [
                permute_to_N_HWA_K(x, self.num_classes) for x in det_cls_query
            ]
            det_bbox_query = [permute_to_N_HWA_K(x, 4) for x in det_bbox_query]
            query_anchors = anchors_value
        else:
            if not self.qInfer.initialized:
                cls_weights, cls_biases, bbox_weights, bbox_biases = (
                    self.det_head.get_params()
                )
    ...
    def det_loss(
        ...
        all_anchor_lists = [
            torch.cat([x.tensor.reshape(-1, 4) for _ in range(batch_size)])
            for x in all_anchors
        ]
        gt_clsses_list = [gt_classes[s:e] for s, e in zip(start_inds, end_inds)]
        gt_anchors_targets_list = [
            gt_anchors_targets[s:e] for s, e in zip(start_inds, end_inds)
        ]
        valid_idxs_list = [valid_idxs[s:e] for s, e in zip(start_inds, end_inds)]
        foreground_idxs_list = [
            foreground_idxs[s:e] for s, e in zip(start_inds, end_inds)
        ]
        ...
        loss_cls = [
            ...
            * sigmoid_focal_loss_jit(
            ...
            for w, x, g, v, f, alpha, gamma in zip(
                cls_weights,
                pred_logits,
                gt_clsses_list,
                valid_idxs_list,
        ...
        if self.use_giou_loss:
            ...
            loss_box_reg = [
                w
                * smooth_l1_loss(
                    x[f], g[f].detach(), beta=self.smooth_l1_loss_beta, reduction="sum"
                )
                for w, x, g, f in zip(
                    reg_weights,
                    pred_deltas,
    ...
    def query_loss(self, gt_small_obj, pred_small_obj, gammas, weights):
        ...
        loss = sum(
            [
                sigmoid_focal_loss_jit(x, y, alpha=0.25, gamma=g, reduction="mean") * w
                for (x, y, g, w) in zip(pred_logits, gts, gammas, weights)
            ]
        )
        ...
        for targets_per_image in targets:
            ...
            if type(self.matcher) == Matcher:
                match_quality_matrix = pairwise_iou(
                    targets_per_image.gt_boxes, all_anchors
                )
                gt_matched_idxs, anchor_labels = self.matcher(match_quality_matrix)
                del match_quality_matrix
            elif (
                type(self.matcher) == LoopMatcher
            ):  # for encoding images with lots of gts
                gt_matched_idxs, anchor_labels = self.matcher(
                    targets_per_image.gt_boxes, all_anchors
                )
            else:
            ...
            gt_anchors_targets.append(
                [gt_anchors_reg_targets_i[s:e] for s, e in zip(start_inds, end_inds)]
            )

        gt_classes = [
            torch.stack([x[i] for x in gt_classes]) for i in range(anchor_layers)
        ]
        gt_anchors_targets = [
            torch.stack([x[i] for x in gt_anchors_targets])
    ...
    def get_query_gt(self, small_anchor_centers, targets):
        ...
        for lind, anchor_center in enumerate(small_anchor_centers):
            ...
            for target_per_image in targets:
                ...
                center_dis, minarg = get_anchor_center_min_dis(
                    small_boxes.gt_boxes.get_centers(), anchor_center
                )
                ...
                if len(small_boxes) != 0:
                    ...
                    small_obj_target[
                        center_dis
                        < min_small_target_scale * self.small_center_dis_coeff[lind]
                    ] = 1
    ...
    def inference(
        ...
        retina_box_cls = [
            permute_to_N_HWA_K(x, self.num_classes) for x in retina_box_cls
        ]
        ...
        for img_idx, image_size in enumerate(image_sizes):
            ...
            retina_box_cls_per_image = [
                box_cls_per_level[img_idx] for box_cls_per_level in retina_box_cls
            ]
            retina_box_reg_per_image = [
                box_reg_per_level[img_idx] for box_reg_per_level in retina_box_delta
            ]
            small_det_logits_per_image = [
                small_det_cls_per_level[img_idx]
                for small_det_cls_per_level in small_det_logits
            ]
            small_det_reg_per_image = [
                small_det_reg_per_level[img_idx]
                for small_det_reg_per_level in small_det_delta
            ]

            if (
                len(small_det_anchors) == 0
                or type(small_det_anchors[0]) == torch.Tensor
            ):
                small_det_anchor_per_image = [
                    small_det_anchor_per_level[img_idx]
                    for small_det_anchor_per_level in small_det_anchors
                ]
            else:
    ...
    def inference_single_image(
        ...
        with autocast(False):
            ...
            boxes_all, scores_all, class_idxs_all = self.decode_dets(
                all_cls, all_delta, all_anchors
            )
            ...
            if self.use_soft_nms:
                keep, soft_nms_scores = self.soft_nmser(
                    boxes_all, scores_all, class_idxs_all
                )
            else:
                keep = batched_nms(
