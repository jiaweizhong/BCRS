...
class QueryInfer(object):
    ...
    def _make_sparse_tensor(
        ...
        good_idx = (
            (sparse_ys >= 0) & (sparse_ys < fh) & (sparse_xs >= 0) & (sparse_xs < fw)
        )
        ...
        sparse_features = (
            feature_value.view(fc, -1).transpose(0, 1)[sparse_inds].view(-1, fc)
        )
    ...
    def _make_spconv(self, weights, biases):
        ...
        for i in range(len(weights)):
            ...
            filter.weight.data[:] = (
                weights[i].permute(2, 3, 1, 0).contiguous()[:]
    ...
    def _make_conv(self, weights, biases):
        ...
        for i in range(len(weights)):
            ...
            filter = torch.nn.Conv2d(
                in_channel, out_channel, k_size, 1, padding=k_size // 2
            )
    ...
    def run_qinfer(self, model_params, features_key, features_value, anchors_value):
        ...
        for i in range(len(features_value) - 1, -1, -1):
            x, last_ys, last_xs, inds, selected_anchors, n_inds = (
                self._make_sparse_tensor(
                    query_logits, last_ys, last_xs, anchors_value[i], features_value[i]
                )
            )
            n_inds_all.append(n_inds)
            if x == None:
                break
            cls_result = self._run_spconvs(x, self.cls_spconv).view(
                -1, self.anchor_num * self.num_classes
            )[inds]
            bbox_result = self._run_spconvs(x, self.bbox_spconv).view(
                -1, self.anchor_num * 4
            )[inds]
