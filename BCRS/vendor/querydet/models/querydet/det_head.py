...
class RetinaNetHead_3x3(nn.Module):
    def __init__(self, cfg, in_channels, conv_channels, num_convs, num_anchors):
        ...
        for i in range(self.num_convs):
            cls_layer = nn.Conv2d(
                channels, conv_channels, kernel_size=3, stride=1, padding=1
            )
            bbox_layer = nn.Conv2d(
                channels, conv_channels, kernel_size=3, stride=1, padding=1
            )
    ...
    def get_params(self):
        ...
        bbox_weights = [x.weight for x in self.bbox_subnet] + [
            self.bbox_pred.weight.data
        ]
...
class Head_3x3(nn.Module):
    def __init__(
        ...
        self.pred_net = nn.Conv2d(
            channels, pred_channels, kernel_size=3, stride=1, padding=1
        )
...
class RetinaNetHead_3x3_MergeBN(nn.Module):
    ...
    def _bn_convert(self):
        ...
        for i in range(self.num_convs):
            ...
            self.cls_subnet[i].weight.data = self.cls_subnet[
                i
            ].weight.data * cls_bn_scale.view(-1, 1, 1, 1)
            self.cls_subnet[i].bias = torch.nn.Parameter(cls_bn_bias)
            self.bbox_subnet[i].weight.data = self.bbox_subnet[
                i
            ].weight.data * bbox_bn_scale.view(-1, 1, 1, 1)
    ...
    def get_params(self):
        ...
        bbox_ws = [x.weight.data for x in self.bbox_subnet] + [
            self.bbox_pred.weight.data
        ]
...
class Head_3x3_MergeBN(nn.Module):
    ...
    def _bn_convert(self):
        ...
        for i in range(self.num_convs):
            ...
            self.subnet[i].weight.data = self.subnet[i].weight.data * bn_scale.view(
                -1, 1, 1, 1
            )
