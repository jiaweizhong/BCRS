...
- **Current status: not re-resolved yet, deliberately.** `models/cfg/esod/uavdt_yolov5m.yaml` still reads `nc: 3` as of this writing. Flipping it back to `nc: 1` is not, by itself, a valid fix — the label `.txt` files under `/root/autodl-tmp/UAVDT_processed/labels/` still contain real `0`/`1`/`2` class ids; changing only the model head's `nc` without rewriting every label file to force class `0` would break (index mismatch) or silently misrun, not correctly reproduce single-class training. Doing this properly requires the same treatment VisDrone got in `HESOD-Experiment-Plan.md`'s "UAVDT status" note: fetch UAVDT from the *official* raw source (ESOD's README links a specific Google Drive file, not this project's current `/root/autodl-tmp/UAVDT_processed`, whose provenance is suspected third-party and already found to have a separate, unrelated box-quality defect — see that note) and run `prepare_uavdt()` on it directly, which produces correctly-collapsed single-class labels as a side effect of the same conversion that also fixes the box-quality issue. Patching just the `nc` value against the current, already-suspect data would be a partial, likely-to-be-redone fix; deferred until UAVDT work resumes (paused as of this writing, VisDrone takes priority).
