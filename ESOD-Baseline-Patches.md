...
- **Round 3, in progress: is the paper's number single-class or 3-class?** The gap above is large enough (~1.8-2.3x) to be suspicious rather than a genuine improvement. Investigated whether UAVDT's vendored MATLAB toolkit (`evaluation/UAV-benchmark-MOTD_v1.0/utils/evalRes.m`, default `thr=.7`, VOC-style `VOCap.m` AP integration, not COCO 0.5:0.95 averaging) explains it — confirmed real (our own AP at IoU=0.70 specifically, via the new `scripts/esod_baseline/eval_at_iou_thresholds.py`, is 52.2%) but does not fully close the gap (52.2% vs. 22.5% is still ~2.3x), and `VOCap.m`'s all-point interpolation is not meaningfully different from pycocotools' 101-point method, so that's not it either. Current leading hypothesis instead: Table I's other baselines in the same row (FasterRCNN 11.0/23.4, ClusDet 13.7/26.5, DMNet 14.7/24.6, CDMNet 16.8/29.1, CEASC 17.1/30.9) are widely-cited numbers from the standard **3-class** UAVDT-DET literature — if the whole table (including ESOD's own 22.5/40.7) is on that same standard protocol for a fair comparison, then `prepare_uavdt()`'s single-class collapse in the *publicly released* code may simply not match whatever produced the paper's actual Table I number. Added `--keep-classes` to `scripts/data_prepare.py` (all three trees) to preserve real 0-indexed categories instead of forcing class 0, and `models/cfg/esod/uavdt_yolov5m_nc3.yaml` (nc=3, kept alongside the nc=1 default, not replacing it) to test this directly. Not yet run.
- **Reporting caveat (still open):** do not treat either the nc=1 or (once run) nc=3 local number as verified-equivalent to the paper's 22.5/40.7 until this is resolved — report paper-reported and locally-reproduced numbers separately.
